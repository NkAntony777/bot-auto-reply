...
- **跨屏 DPI 往返后的输入僵死**（2026-08-17 实测）：bot 退出把微信从虚拟屏
  （100% 缩放）还回主屏（125%）后，偶发窗口渲染正常但**鼠标点击/关闭消息全部无响应**
  （SC_CLOSE/WM_CLOSE 也不理，命中测试却正常）——Qt 在快速 DPI 往返后的输入处理僵死。
  临时处置：托盘图标点两下（复活切换）或强杀 Weixin.exe（WAL 数据库崩溃安全）。
  待办：restore_wechat_to_primary 之后自动做一次托盘复活切换，重置窗口输入状态。
